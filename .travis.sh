...
if [ "x$TEST_DOC" = "xYES" ]; then
    ...
    THEANO_FLAGS="$FLAGS",blas.ldflags="-lblas -lgfortran",-warn.ignore_bug_before=all,on_opt_error=raise,on_shape_error=raise coverage run --source=pylearn2 -m nose.__main__ $PART
fi
